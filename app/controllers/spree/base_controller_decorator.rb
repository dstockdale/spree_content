Spree::BaseController.class_eval do
  ...
  def render_page_if_exists
    ...
    if @content = Spree::Arrangement.visible.find_by_slug(request.path)
      
      if @content.layout && !@content.layout.empty?
        render :template => 'content/show', :layout => @content.layout
      else
